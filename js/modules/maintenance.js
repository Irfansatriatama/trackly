...
async function _openTicketModal(ticketId) {
  ...
  const viewers    = _members.filter(m=>m.status!=='inactive'&&m.role==='viewer');
...
function _getStatusVariant(s) {
  return { backlog:'neutral', in_progress:'info', awaiting_approval:'warning', on_check:'info', need_revision:'warning', completed:'success', canceled:'danger', on_hold:'neutral',
    open:'warning', resolved:'success', closed:'neutral', rejected:'danger' }[s]||'neutral';
}
